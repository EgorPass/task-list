 ...
 * @returns {void}
 */
export	const deleteFileFromStorage = async (path, file) => {
		const storage = storageRef(getStorage(), path)
		deleteObject(storage)
			.then(() => {
				console.log("этот файл: ", file, " удален")
			})
			.catch(() => {
				console.log("что то пошло не так, ", file, " не удален")
		})
}
 ...
 * @returns 
 */
export const uploadFileToStorage = (path, file) => {
	const storage = storageRef(getStorage(), `${path}`)
 ...
 */
export const downlaodFileFromStorage = async (path, name) => {
 ...
 */
export const monitor = async (callback) => {
